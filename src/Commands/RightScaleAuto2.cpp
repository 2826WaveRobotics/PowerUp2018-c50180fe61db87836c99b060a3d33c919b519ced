...
// it from being updated in the fut+ure.



#include "RightScaleAuto2.h"
...
RightScaleAuto2::RightScaleAuto2() {
	...
	AddParallel(new SetKirby(1, 0.5));
	...
	//Back away as time allows
	AddSequential(new AutoDistance(50, 0.8, -178));
	AddParallel(new SetElevator(0));
	AddParallel(new SetElbow(160));
	AddParallel(new AutoRotate(-130));
	AddSequential(new SetElevator(0));
	AddParallel(new SetKirbyKlaw(true));

	AddSequential(new AutoDrive(-40, -.4, -130, true));
